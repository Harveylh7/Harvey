...
void call_open()
{
    ...
    Process *p = arch.pcurrent; // 获取当前进程的指针
...
void call_read()
{
    ...
    u32 ret=fp->read(info->ptr,buf,size);//读取的字节数
	info->ptr=info->ptr + ret;//移动文件指针
	arch.setRet(ret);
...
void call_symlink()
{
	...
	int ret=fsm.link(oldpath,path);//实际上是调用文件系统的接口
	arch.setRet((u32)ret);
	return;
}
...
void call_getdents()
{
    ...
    openfile* info = p->getFileInfo(fd);
    int i=0;
    File* child=fp->getChild();// 拿到fp下的子文件

    while (child!=NULL){
		if (i==(info->ptr)){//ptr是u32类型
